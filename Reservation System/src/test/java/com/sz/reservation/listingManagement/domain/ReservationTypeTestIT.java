...
//THIS CLASS WILL TEST THAT THE VALUES ON THE Reservation Type TABLE MATCHES THE ENUM ONES

@WebAppConfiguration
@ActiveProfiles(profiles = {"test","default"})
@ContextConfiguration(classes = {RootConfig.class, ListingConfig.class})
@ExtendWith(SpringExtension.class)
@DisplayName("Reservation Type Enum , Database SYNC test")
class ReservationTypeTestIT {
    ...
    public void AllEnumValues_ShouldMatch_DB(){
        //arrange
        String sql = "SELECT * FROM reservation_type";
        ...
        jdbcTemplate.query(sql, new RowCallbackHandler() {
            ...
            public void processRow(ResultSet rs) throws SQLException {
                resultMap.put(rs.getInt("id_reservation_type"),rs.getString("type"));
            }
        });
        ...
        for (int i = 0; i<reservationTypes.length; i++){
            String type = reservationTypes[i].name();
            int id = reservationTypes[i].getId();
