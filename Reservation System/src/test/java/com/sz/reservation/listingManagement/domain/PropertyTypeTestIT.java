...
//THIS CLASS WILL TEST THAT THE VALUES ON THE PROPERTY TYPE TABLE MATCHES THE ENUM ONES

@WebAppConfiguration
@ActiveProfiles(profiles = {"test","default"})
@ContextConfiguration(classes = {RootConfig.class, ListingConfig.class})
@ExtendWith(SpringExtension.class)
@DisplayName("Property Type Enum , Database SYNC test")
class PropertyTypeTestIT {
    ...
    public void AllEnumValues_ShouldMatch_DB(){
        ...
        jdbcTemplate.query(sql, new RowCallbackHandler() {
            ...
            resultMap.put(rs.getInt("id_property_type"),rs.getString("type"));
            }
        });

        //assert
        for (int i = 0; i<propertyTypes.length; i++){
            String type = propertyTypes[i].name();
            int id = propertyTypes[i].getId();
